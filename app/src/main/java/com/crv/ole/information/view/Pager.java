...
public class Pager extends View {
    ...
    public boolean isInCenter(float x) {

        return (mWidth / 2) / 3 < x && x < (mWidth * 5 / 6);
    ...
    public void calcCornerXY(float x, float y) {
        if (x <= (mWidth / 2) / 3) {
            mCornerX = 0;
        } else if ((mWidth * 5 / 6) <= x && x <= mWidth) {
            if (mSize == 1) {//单张图片会引起初始化坐标混乱问题，在此判断是否为单张
                mCornerX = 0;
            } else {
                mCornerX = mWidth;
            }
    ...
    public boolean doTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_MOVE) {
            Log.e("Pager", "ACTION_MOVE");
            mTouch.x = event.getX();
            mTouch.y = event.getY();
            if (event.getX() < mWidth && event.getX() > (mWidth / 2) / 3) {
                mTouch.x = (mWidth / 2) / 3;
            } else if (event.getX() < (mWidth * 5 / 6)) {
                mTouch.x = (mWidth * 5 / 6);
            }
            this.postInvalidate();
        }
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            Log.e("Pager", "ACTION_DOWN");
            mTouch.x = event.getX();
            mTouch.y = event.getY();

...
/**
 * 个人中心 - 我的收藏 - 咨询文件夹列表适配器
 * Created by wj_wsf on 2017/7/17.
 */

public class CollectionInformationFolderListAdapter extends BaseAdapter {
    private Context context;
    private List<CollectionFolderListData.FolderData> dataList;
    private int tag = 0;//是否为编辑状态？0：未编辑，1：编辑

    public CollectionInformationFolderListAdapter(Context context, List<CollectionFolderListData.FolderData> messageDataList) {
    ...
    public View getView(int position, View v, ViewGroup parent) {
        ...
        if (position != dataList.size() - 1)
            LoadImageUtil.getInstance().loadBackground(viewHolder.collection_item_layout, dataList.get(position).getArticleFileImage());
        else
            viewHolder.collection_item_layout.setBackgroundResource(Integer.valueOf(dataList.get(position).getArticleFileImage()));
        ...
        viewHolder.name.setText(dataList.get(position).getArticleFileName());

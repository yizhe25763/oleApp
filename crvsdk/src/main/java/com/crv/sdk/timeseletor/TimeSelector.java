package com.crv.sdk.timeseletor;
...
/**
 * Created by Tom_Yang on 2017/07/12.
 */
public class TimeSelector {
    ...
    private void initTimer() {
        ...
        calendar.set(Integer.valueOf(year.get(year.size()/2)),startMonth-1,startDay);
        selectedCalender.setTime(calendar.getTime());
        loadComponent();
    ...
    public void loadComponent() {
        ...
        year_pv.setSelected(year.size()/2);
    ...
    public void refreshComponent(String birthday) {

        String yearStr = birthday.substring(0, 4);
        String monthStr = birthday.substring(5, 7);
        String dayStr = birthday.substring(8, 10);
        int yearIndex = year.indexOf(yearStr);
        if (yearIndex == -1) return;
        int monthIndex = month.indexOf(monthStr);
        if (monthIndex == -1) return;
        int dayIndex = day.indexOf(dayStr);
        if (dayIndex == -1) return;
        year_pv.setSelected(yearIndex);
        month_pv.setSelected(monthIndex);
        day_pv.setSelected(dayIndex);
        excuteScroll();
        selectedCalender.setTime(DateUtil.parse(birthday,FORMAT_STR));
    }
